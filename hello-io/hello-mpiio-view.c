...
int main(int argc, char **argv)
{
    ...
    /* we can deal with variable length strings but for the sake of this
     * exercise it will be simpler if all strings are the same length */
    snprintf(buf, BUFSIZE, "Hello from rank %3d of %3d\n", rank, nprocs);
    len = strlen(buf);

    /* noncontiguous in file requres a "file view".  In this example we will
     * have each process write their strings such that all the 'H' comes first,
     * then all the 'e' ... etc
    ...
    /* each process will write to its own "view" of the file: for two processes
     * it would look like:
     * - - - - - - - - - - - ...
     ...
     *  H e l l o  f r o m ...
     *
     * and we set up displacements so that each process does not step on any
     * other process: in the file it looks like
     * HHeelloo  ffrroomm... */
    for (int i=0; i< len; i++)
