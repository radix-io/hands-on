...
int write_data(char *filename)
{
    ...
    ret = MPI_File_write(fh, &data, sizeof(data), MPI_BYTE, NULL);
    if (ret != MPI_SUCCESS) goto fn_error;

    ret = MPI_File_write(fh, array, XDIM*YDIM*sizeof(int), MPI_BYTE, NULL);
    if (ret != MPI_SUCCESS) goto fn_error;
...
int main(int argc, char **argv)
{
    ...
    // use testfile for DFUSE and IL
    // use daos:testfile for testing DAOS ADIO
    //ret = write_data("testfile");
    ret = write_data("daos:testfile");
